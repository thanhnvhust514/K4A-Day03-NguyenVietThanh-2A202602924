                ...
                if obs_data.get("status") == "SUCCESS":
                    if "message" in obs_data:
                        ...
                        if "gpa" in d or "full_name" in d:
                            ...
                            final_answer = (
                                f"Thông tin sách '{d.get('title', '')}' (Mã: {obs_data.get('book_id', '')}): "
                                f"Tác giả: {d.get('author', '')}, Thể loại: {d.get('category', '')}, "
                                f"Tình trạng: {d.get('status', '')}, Vị trí: {d.get('location', '')}."
                            )
                        else:
                            final_answer = f"Dữ liệu từ MCP Server: {json.dumps(d, ensure_ascii=False)}"
                    else:
                        final_answer = f"Đã hoàn tất xử lý qua MCP Server: {json.dumps(obs_data, ensure_ascii=False)}"
                elif obs_data.get("status") == "NOT_FOUND":
                    final_answer = obs_data.get("message", "Không tìm thấy thông tin theo yêu cầu.")
                else:
